...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for corner in unvisitedCorners:
        distance = abs(pacmanPos[0] - corner[0]) + abs(pacmanPos[1] - corner[1])
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if not foodCoordinates:
        return 0  # If there is no remaining food, the heuristic is 0

    # Calculate the sum of Manhattan distances to the closest food pellet
    distances = [abs(pacmanPosition[0] - food[0]) + abs(pacmanPosition[1] - food[1]) for food in foodCoordinates]
